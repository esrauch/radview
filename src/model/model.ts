...
import { activityToString, CompactActivity } from "./activity.js"

export type LatLon = { lat: number, lon: number }

export type LatLonCompact = [number, number]

export type CitySelect = 'none' | 'high' | 'clip_somerville' | 'clip_cam'

class Model extends Listenable {
    activities: Array<Readonly<CompactActivity>> = []
    desc = ''

    current: Array<Readonly<CompactActivity>> = []
    ...
    init(activities: CompactActivity[], waters: Water[], paths: Way[]) {
